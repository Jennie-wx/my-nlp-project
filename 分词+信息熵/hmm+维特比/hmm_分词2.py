...
def text_to_state(file="pku_training.utf8"):  # 将原始的语料库转换为 对应的状态文件

    if os.path.exists("train_state.txt"):  # 如果存在该文件, 就直接退出
        return
    all_data = open(file, "r", encoding="utf-8").read().split("\n")  # 打开文件并按行切分到  all_data 中 , all_data  是一个list
    with open("train_state.txt", "w", encoding="utf-8") as f:    #  代开写入的文件
...
class HMM:
    def __init__(self,file_text = "pku_training.utf8",file_state = "train_state.txt"):
...
if __name__ == "__main__":
    ...
    hmm = HMM()
    hmm.train()
    result1 = viterbi_t(text,hmm)
    print(result1)
    for i in range(len(test_text)):
        result = viterbi_t(test_text[i],hmm)
        print(result)
